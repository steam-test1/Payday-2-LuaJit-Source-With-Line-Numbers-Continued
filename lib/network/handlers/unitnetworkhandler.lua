...
function UnitNetworkHandler:long_dis_interaction(target_unit, amount, aggressor_unit, secondary)
	...
	if (not target_is_criminal or not aggressor_is_criminal or (not target_unit:brain() or target_unit:brain().on_long_dis_interacted) and amount == 1) and (amount ~= 0 or not target_is_civilian or not aggressor_is_criminal or self._verify_in_server_session()) and target_unit:brain() then
