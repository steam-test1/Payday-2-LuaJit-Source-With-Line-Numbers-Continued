...
function EnvironmentFire:update(unit, t, dt)
	if self._burn_duration <= 0 then
		...
		if self._burn_tick_counter > self._burn_tick_period then
			self:_do_damage()
		end
	end
end
...
function EnvironmentFire:save(data)
	...
	state.user_unit = alive(self._user_unit) and self._user_unit:id() ~= -1 and self._user_unit or nil
	state.weapon_unit = alive(self._weapon_unit) and self._weapon_unit:id() ~= -1 and self._weapon_unit or nil
	state.burn_duration = self._burn_duration
	...
	state.dot_data = self._dot_data
	state.fire_alert_radius = self._fire_alert_radius
	state.no_fire_alert = self._no_fire_alert
