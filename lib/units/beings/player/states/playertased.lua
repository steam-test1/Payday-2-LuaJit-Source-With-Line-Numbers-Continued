	...
	if action_wanted then
		if not action_forbidden then
			...
			if self._equipped_unit then
				...
				if (not weap_base:out_of_ammo() or input.btn_primary_attack_press) and (not weap_base.clip_empty or not weap_base:clip_empty() or fire_mode == "single" and input.btn_primary_attack_press) then
					if self._num_shocks > 1 and weap_base.can_refire_while_tased and not weap_base:can_refire_while_tased() then
						-- Nothing
